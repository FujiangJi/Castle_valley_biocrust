...
def main():
    ...
    frac_cols = ['frac_Vegetation', 'frac_DkCy', 'frac_Lichen',
                 'frac_LtCy', 'frac_Moss']
    ...
    # frac_Vegetation in this data version already includes Litter + Live + Dead
    ...
    target_cols_5 = ['frac_Vegetation', 'frac_DkCy',
    ...
    # Mask bad bands as NaN to create visual gaps
    bad_mask = np.zeros(len(mix_wavelengths), dtype=bool)
    for lo, hi in bad_bands:
        bad_mask |= (mix_wavelengths >= lo) & (mix_wavelengths <= hi)
    ...
    for idx, treat in enumerate(treat_order):
        ...
        # Shade bad bands
        for lo, hi in bad_bands:
            ax.axvspan(lo, hi, color='lightgray', alpha=0.1, zorder=0)
