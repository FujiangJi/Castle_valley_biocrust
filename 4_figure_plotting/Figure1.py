...
def main():
    ...
    inset_lr = 0.22   # left-right inset
    inset_top = 0.03  # top inset (smaller to include vegetation)
    inset_bot = 0.31  # bottom inset (larger to avoid circles)
    ...
    # Cyan box aligned with grid edges (not the full MRR)
    g_tl = origin
    g_tr = origin + v_r
    g_br = origin + v_r + v_d
    g_bl = origin + v_d
    ...
    # ===================================================================
    # Panel E: 5 biocrust photos at bottom
    # ===================================================================
    pic_dir = root / "1_research_area" / "Pictures"
    photos = [
        ('0_Overview.jpg', 'Overview'),
        ('0_LtCy.jpeg', 'LtCy'),
        ('0_DkCy.jpeg', 'DkCy'),
        ('0_Moss.jpeg', 'Moss'),
        ('0_Lichen.jpeg', 'Lichen'),
    ]

    # Square photos: width = height in figure coords, adjusted for fig aspect ratio
    photo_w = (1.0 - margin_l - margin_r - 4 * gap) / 5
    photo_h = photo_w * (fig_w / fig_h)  # square in display
    photo_y = 0.005 - photo_h  # minimal gap to upper figure

    for i, (fname, label) in enumerate(photos):
        x_pos = margin_l + i * (photo_w + gap)
        ax_photo = fig.add_axes([x_pos, photo_y, photo_w, photo_h])
        img = plt.imread(str(pic_dir / fname))
        # Crop to square (center crop)
        h, w = img.shape[:2]
        s = min(h, w)
        y0 = (h - s) // 2
        x0 = (w - s) // 2
        img_sq = img[y0:y0+s, x0:x0+s]
        ax_photo.imshow(img_sq, aspect='equal')
        ax_photo.set_xticks([])
        ax_photo.set_yticks([])
        style_spines(ax_photo)
        # Label in top-left corner
        ax_photo.text(0.04, 0.96, label, transform=ax_photo.transAxes,
