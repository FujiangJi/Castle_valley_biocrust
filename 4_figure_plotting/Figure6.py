"""
Figure 6: Combined figure per block
Top: 2x2 heatmaps (4 treatments, measured vs estimated for 3 targets)
Bottom: 1x3 residual boxplots
"""
...
def main():
    ...
    # Fine-tuned 1D-CNN, bilinear mixing (higher accuracy than linear)
    df = pd.read_csv(cnn_dir / "final_model_estimation_bilinear_3scheme.csv")
    ...
    heatmap_labels = ["Senescent Vegetation", "Late Succ.", "Early Succ."]
    treats = ["Control", "AltP", "Warmed", "AltP+Warmed"]
    plots_list = ["B1", "B2", "B3", "B4", "B5"]
    target_colors = ['#6a51a3', '#238b45', '#e6550d']  # Darker Purple, Green, Orange
    cmaps_col = ['Purples', 'Greens', 'Oranges']
    ...
    for p in plots_list:
        ...
        # Panel A title
        fig.text(-0.05, 1.08, f'a. Measured vs Estimated fractional cover ({p})',
        ...
        for t_i, tr in enumerate(treats):
            ...
            inner = gridspec.GridSpecFromSubplotSpec(
                2, 3, subplot_spec=gs_heat[row_t, col_t],
            ...
            for i in range(2):
                for j in range(3):
                    ...
                    if i == 0:
                        ax.set_title(heatmap_labels[j], fontsize=9, fontweight='bold')
                    if j == 0:
                        ax.set_ylabel(labels2[i], fontsize=9, fontweight='bold',
                                      fontstyle='italic')
                    k += 1

            # Subtitle: (A.1), (A.2), etc.
        ...
        # Panel B title
        fig.text(-0.05, 0.32, f'b. Prediction residuals ({p})',
                 fontsize=12, fontweight='bold', va='top', ha='left')

        # === Bottom: 1x3 residual boxplots ===
        gs_box = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=outer[1],
                                                   wspace=0.25)
        ...
        for t_idx, (tcol, pcol, label) in enumerate(zip(target_cols, pred_cols, heatmap_labels)):
            ...
            ax.set_xticklabels(treats, fontsize=10, rotation=0, ha='center')
            ax.set_ylabel('Residual (%)', fontsize=11, fontweight='bold')
            ax.set_xlabel('Treatments', fontsize=11, fontweight='bold')
        ...
        # --- Save ---
        out_file = out_dir / f'Figure6_{p}.png'
