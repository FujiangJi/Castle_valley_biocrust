...
def synthesize_one_pixel(lib, components, band_cols,
                         ...
                         scale_std=0.03):
    """
    Improved synthetic pixel generation with:
    - Gaussian additive noise (sensor noise simulation)
    - Per-endmember multiplicative scaling (intra-class spectral variability)
    - Variable Dirichlet alpha for diverse mixture types
    """
    ...
    # 3) For each component, pick a random pure spectrum and add variability
    pure_spectra = []
    for comp in comps_selected:
        ...
        # Multiplicative scaling to simulate intra-class variability
        if scale_std > 0:
    ...
    # 5) Linear mixing
    mixed = (abundances[:, None] * pure_spectra).sum(axis=0)

    # 6) Additive Gaussian noise (sensor noise)
    if noise_std > 0:
        mixed = mixed + rng.normal(0, noise_std, size=n_bands)

    # 7) Physical constraint [0, 1]
    mixed = np.clip(mixed, 0, 1)

    # 8) Build abundance dictionary
...
synth.to_csv("../1_data/Processed_data/synthetic_mixtures_linear.csv", index=False)
print(f"Saved {len(synth)} synthetic samples to synthetic_mixtures_linear.csv")
