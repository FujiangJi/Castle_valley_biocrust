...
# 1. CONFIGURATION
# ==========================================
csv_path = "../1_data/Processed_data/synthetic_mixtures_bilinear.csv"  # bilinear synthetic data
model_saved_path = "../2_saved_models/pretrained_spectral_cnn_bilinear_3scheme.pt"
result_path = "../3_results/pretrained_model_estimation_bilinear_3scheme.csv"
loss_path = "../3_results/pretrained_model_loss_bilinear_3scheme.csv"
batch_size = 32
lr = 0.001
n_epoches = 600
test_size = 0.2
...
def merge_targets(df):
    """Merge 6 raw targets into 3 ecological groups (Vegetation column = Litter + Vegetation)."""
    df["frac_Vegetation"] = df["frac_Litter"] + df["frac_Vegetation"]
    df["frac_late_successional"] = df["frac_DkCy"] + df["frac_Lichen"] + df["frac_Moss"]
    df["frac_early_successional"] = df["frac_LtCy"]
    df = df.drop(columns=["frac_Litter", "frac_DkCy",
                           "frac_Lichen", "frac_Moss", "frac_LtCy"], errors='ignore')
    return df
...
def train_one_epoch(model, loader, optimizer, loss_fn, noise_std=0.0, scale_std=0.0):
    model.train()
    total_loss = 0.0
    for X_batch, y_batch in loader:
        X_batch, y_batch = X_batch.to(device), y_batch.to(device)
        if scale_std > 0:
            scale = 1.0 + scale_std * torch.randn(X_batch.size(0), 1, 1, device=device)
...
def main():
    ...
    print("\nStart training 1D-CNN bilinear_3scheme...")
    for epoch in range(1, n_epoches + 1):
        train_loss = train_one_epoch(model, train_loader, optimizer, loss_fn, noise_std=0.0, scale_std=0.0)
